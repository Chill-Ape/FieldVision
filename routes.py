...
@app.route('/api/vegetation-index', methods=['POST'])
def get_vegetation_index():
    """
    Fetch vegetation index image from Sentinel Hub API
    
    POST: Accepts custom parameters in JSON format: 
    {
        "bbox": [lng1, lat1, lng2, lat2],
        "index_type": "ndvi|ndre|moisture|evi|ndwi|chlorophyll",
        "field_id": optional_field_id,
        "geometry": optional_polygon_geometry
    }
    ...
    ndvi_fetcher = NDVIFetcher(auth)
    
    # Get request data
    data = request.get_json() or {}
    bbox = data.get('bbox', [-104.0522, 39.7392, -104.0422, 39.7492])  # Default Denver area
    ...
    if index_type not in valid_indices:
        return jsonify({
            ...
            "message": f"Index must be one of: {', '.join(valid_indices)}"
        }), 400
    ...
    if not ndvi_fetcher.validate_bbox(bbox):
        return jsonify({
            ...
            "message": "Bounding box must contain [min_lng, min_lat, max_lng, max_lat]"
        }), 400
    
    try:
        # Calculate optimal dimensions
        width, height = ndvi_fetcher._calculate_optimal_dimensions(bbox)
        
        # Fetch vegetation index image from API
        image_data = ndvi_fetcher.fetch_ndvi_image(bbox, width, height, geometry, index_type)
        ...
        if image_data:
            ...
            filename = f"{index_names.get(index_type, 'VegIndex')}_{bbox[0]}_{bbox[1]}.png"
            ...
            return jsonify({
                "error": f"Failed to fetch {index_type.upper()} image",
                "message": "Could not retrieve satellite data. Check logs for details."
            }), 500
            
    except Exception as e:
        logger.error(f"Error fetching {index_type} image: {e}")
        return jsonify({
            "error": f"Failed to fetch {index_type.upper()} image",
            "message": str(e)
        }), 500
